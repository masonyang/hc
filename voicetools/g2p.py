...
class PhonetisaurusG2P(object):
    ...
    def execute(cls, fst_model, input, is_file=False, nbest=None):
        ...
        if proc.returncode != 0:
            print("Command '%s' return with exit status %d",
                         ' '.join(cmd))
    ...
    def __init__(self, fst_model=None, nbest=None):
        ...
        print("Using FST model: '%s'", self.fst_model)

        self.nbest = nbest
        if self.nbest is not None:
            print("Will use the %d best results.", self.nbest)
    ...
    def translate(self, words):
        if type(words) is str or len(words) == 1:
            print('Converting single word to phonemes')
            output = self._translate_word(words if type(words) is str
                                          else words[0])
        else:
            print('Converting %d words to phonemes', len(words))
            output = self._translate_words(words)
        print('G2P conversion returned phonemes for %d words',
                           len(output))
        return output
