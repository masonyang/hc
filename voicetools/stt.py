...
class PocketSphinxSTT(AbstractSTTEngine):
    ...
    def __init__(self, vocabulary, hmm_dir="/usr/local/share/" +
        ...
        print("Initializing PocketSphinx Decoder with hmm_dir " +
                           "'%s'", hmm_dir)
        ...
        if missing_hmm_files:
            print("hmm_dir '%s' is missing files: %s. Please " +
    ...
    def transcribe(self, fp):
        ...
        print('PocketSphinx 识别到了：%r', transcribed)
        return transcribed
...
class BaiduSTT(AbstractSTTEngine):
    ...
    def transcribe(self, fp):
        demo_wav = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'demo.wav')
        try:
...
class SnowboySTT(AbstractSTTEngine):
    ...
    def transcribe(self, fp):
        ...
        if ans > 0:
            print('snowboy 识别到了: %r', self.hotword)
            return [self.hotword]
        else:
            return []
...
class GoogleSTT(AbstractSTTEngine):
    ...
    def transcribe(self, fp):
        ...
        if not self.api_key:
            print('API key missing, transcription request ' +
                                  'aborted.')
            return []
        elif not self.language:
            print('Language info missing, transcription ' +
                                  'request aborted.')
            return []
        ...
        try:
            ...
            print('Request failed with http status %d',
                                  r.status_code)
            if r.status_code == requests.codes['forbidden']:
                print('Status 403 is probably caused by an ' +
