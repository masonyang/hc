...
# except ImportError:
#     print("Error importing CMUCLMTK plugin. " +
...
class AbstractVocabulary(object):
    ...
    def compiled_revision(self):
        ...
        print("compiled_revision is '%s'", revision)
        return revision
    ...
    def compile(self, phrases, force=False):
        ...
        if not force and self.compiled_revision == revision:
            print('Compilation not neccessary, compiled ' +
                               'version matches phrases.')
            return revision

        if not os.path.exists(self.path):
            print("Vocabulary dir '%s' does not exist, " +
                               "creating...", self.path)
            try:
                os.makedirs(self.path)
            except OSError:
                print("Couldn't create vocabulary dir '%s'",
                                   self.path)
                raise
        try:
            ...
            print("Couldn't write revision file in '%s'",
                               self.revision_file)
            raise
        else:
            ...
            try:
                ...
                print("Fatal compilation Error occured, " +
                                   "cleaning up...")
                try:
